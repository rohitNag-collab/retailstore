...
public class NetPayableAmountService implements INetPayableAmountService {
    ...
    public double calculateNetPayableAmount(Bill bill) {
        ...
        boolean isCustomerMoreThan2YearsOld = user.getCreateDate()
                .isBefore(LocalDate.now().minusYears(2));
        ...
        double percentageDiscount = calculatePercentageDiscount(bill, nonGroceryAmount, isCustomerMoreThan2YearsOld);

        return totalAmount - fixedDiscount - percentageDiscount;
    }

    private double calculatePercentageDiscount(Bill bill,
